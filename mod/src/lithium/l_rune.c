...
lvar_t *rune_predator_speed;
lvar_t *rune_predator_gravity;
lvar_t *rune_predator_closedmg;
lvar_t *rune_predator_closerange;
lvar_t *rune_predator_recloak;
lvar_t *rune_predator_selfdestruct_dmg;
lvar_t *rune_predator_selfdestruct_radius;

int rune_count[NUM_RUNES] = { 0, 0, 0, 0, 0, 0 };
int rune_total = 0;
...
void Rune_InitGame(void) {
	...
	rune_flags = lvar("rune_flags", "63", "##", VAR_RUNE);
	...
	rune_predator_recloak = lvar("rune_predator_recloak", "3.0", "#.##", VAR_RUNE);
	rune_predator_selfdestruct_dmg = lvar("rune_predator_selfdestruct_dmg", "200", "###", VAR_RUNE);
	rune_predator_selfdestruct_radius = lvar("rune_predator_selfdestruct_radius", "250", "###", VAR_RUNE);
}
...
edict_t *Rune_Spawn(vec3_t origin, int type) {
	...
	else if(type & RUNE_PREDATOR)
		rune->s.renderfx = RF_SHELL_HALF_DAM;
...
void Rune_Touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf) {
	...
	if(self->rune & RUNE_PREDATOR)
		Rune_PredatorCloak(other);
...
void Rune_Drop(edict_t *player) {
	...
	if(player->rune & RUNE_PREDATOR)
		Rune_PredatorUncloak(player);
...
int Rune_AdjustDamage(edict_t *targ, edict_t *attacker, int damage) {
	...
	if(attacker->rune & RUNE_PREDATOR) {
		vec3_t diff;
		VectorSubtract(targ->s.origin, attacker->s.origin, diff);
		if(VectorLength(diff) < rune_predator_closerange->value)
			damage *= rune_predator_closedmg->value;
	}
	return damage;
}
...
void Rune_ClientFrame(edict_t *player) {
	...
	if(player->rune & RUNE_PREDATOR && !player->client->predator_cloaked) {
		if(player->client->predator_recloak_time && level.time >= player->client->predator_recloak_time) {
			Rune_PredatorCloak(player);
		}
	}
...
void Rune_FireSound(edict_t *ent) {
	...
	if(ent->rune & RUNE_PREDATOR && ent->client->predator_cloaked)
		Rune_PredatorUncloak(ent);
}
...
qboolean Rune_Give(edict_t *ent, char *name) {
	...
	else if(!Q_stricmp(name, "predator")) {
		ent->rune = RUNE_PREDATOR;
		Rune_PredatorCloak(ent);
	}
	else
		return false;
...
void CTFSay_Team_Rune(edict_t *who, char *buf, unsigned int buflen) {
	...
	else if(who->rune & RUNE_PREDATOR)
		strlcpy(buf, "the Predator rune", buflen);
	else
		strlcpy(buf, "no rune", buflen);
}

void Rune_PredatorCloak(edict_t *ent) {
	ent->client->predator_cloaked = true;
	ent->client->predator_recloak_time = 0;
}

void Rune_PredatorUncloak(edict_t *ent) {
	ent->client->predator_cloaked = false;
	ent->client->predator_cloak_break_time = level.time;
	ent->client->predator_recloak_time = level.time + rune_predator_recloak->value;
}

void Rune_PredatorSelfDestruct(edict_t *self) {
	if(!(self->rune & RUNE_PREDATOR))
		return;
	...
	T_RadiusDamage(self, self, rune_predator_selfdestruct_dmg->value,
		NULL, rune_predator_selfdestruct_radius->value, MOD_PREDATOR);
}
