...
//ZOID
		//WF
		Rune_PredatorSelfDestruct(self);
...
//WF
void
ClientUserinfoChanged2(edict_t *ent, char *userinfo, unsigned int uilen)
{
	char *s;
	int playernum;
	...
	/* spectators are only supported in deathmatch
	   WF: Lithium uses observers, disable standard spectator mode */
	//WF
	ent->client->pers.spectator = false;
	//WF
...
//ZOID
	...
	if (game.maxclients > 1)
	{
		//WF
		char *c, ipstr[32];
		Q_strlcpy(ipstr, Info_ValueForKey(userinfo, "ip"), sizeof(ipstr));
		c = strchr(ipstr, ':');
		if (c)
		{
			*c = 0;
...
ClientThink(edict_t *ent, usercmd_t *ucmd)
{
	...
	else
	{
		...
		if(ent->rune & RUNE_PREDATOR)
			client->ps.pmove.gravity *= rune_predator_gravity->value;
		...
		pm.cmd = *ucmd;
		if(ent->rune & RUNE_PREDATOR) {
			pm.cmd.forwardmove *= rune_predator_speed->value;
			pm.cmd.sidemove *= rune_predator_speed->value;
		}
