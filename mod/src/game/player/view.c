...
P_FallingDamage(edict_t *ent)
{
	...
	/* never take damage if just released grapple or on grapple */
...
G_SetClientEffects(edict_t *ent)
{
	...
	if (ent->client->quad_framenum > level.framenum
		&& (level.framenum & 8 || (ctf_coloredquad && ctf_coloredquad->value) || !(ctf && ctf->value))
		)
	{
		remaining = ent->client->quad_framenum - level.framenum;
		...
		if ((remaining > 30) || (remaining & 4))
		{
			if(ctf && ctf->value && ctf_coloredquad && ctf_coloredquad->value && ent->client->resp.ctf_team == CTF_TEAM1)
			{
				ent->s.effects |= EF_COLOR_SHELL;
