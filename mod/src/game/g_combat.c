		...
		int knockback, int dflags, int mod)
{
	...
	/* Lithium II: rune damage adjustment */
		...
		(targ->movetype != MOVETYPE_STOP))
	{
		...
		{
			apply_knockback(targ, dir, adjusted_knockback * knockback_self->value, 500.0f);
		}
		else
		{
			apply_knockback(targ, dir, adjusted_knockback, 500.0f);
		}
	}

	/* Lithium II: spawn protection */
	...
	/* CTF: check if attacker hurt flag carrier */
	...
	if (take)
	{
		...
		/* Lithium II: rune damage tracking */
